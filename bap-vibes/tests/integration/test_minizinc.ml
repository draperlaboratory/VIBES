...
let op_opcodes1 =
  Tid.Map.of_alist_exn
    [
      (op1.id, [ mov ]); (op2.id, [ mov ]); (blk1.ins.id, []); (blk1.outs.id, []);
    ]
...
let operations1 = List.map ~f:(fun o -> o.id) [ blk1.ins; op1; op2; blk1.outs ]

let oprnd_temps1 = [ t1; t1; t2; t2; t3; t3 ]

(* Put the above IR into a block we can use for a test. *)
let ex1 : Ir.t = { congruent = []; blks = [ blk1 ] }
...
let test_minizinc_ex1 (ctxt : test_ctxt) : unit =
  ...
  let computation =
    ...
    Minizinc.run_minizinc arm_tgt arm_lang ~filepath:model [] ex1
    >>= fun (vir, _) ->
    let get_ops ir =
      let blk = List.hd_exn ir.blks in
      blk.data
    in
    assert_bool "Operations should be in order"
      (List.for_all2_exn
         ~f:(fun o1 o2 -> Tid.equal o1.id o2.id)
         (get_ops vir) (get_ops ex1));
    KB.return obj
  in
  let _ = KB.run Data.cls computation KB.empty in
  ()

let suite = [ "Test Minizinc.run_minizinc" >:: test_minizinc_ex1 ]
