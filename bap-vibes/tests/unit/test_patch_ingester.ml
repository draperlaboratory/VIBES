...
let test_ingest (_ : test_ctxt) : unit =
  ...
  let computation =
    ...
    | [ p ] ->
        ...
        let err =
          Format.asprintf "Expected %a but got %a" Bil.pp expected Bil.pp bil
        in
...
let test_ingest_with_no_patch (_ : test_ctxt) : unit =
  ...
  let computation =
    ...
    Data.Patched_exe.set_patches obj (Data.Patch_set.singleton patch)
    >>= fun _ ->
    Patch_ingester.ingest obj >>= fun _ -> KB.return obj
  in
...
let suite =
  [
    ...
    "Test Patch_ingester.ingest: no patch" >:: test_ingest_with_no_patch;
