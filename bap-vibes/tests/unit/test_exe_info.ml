...
(* Test that [Exe_info.extract] stashes the address size in the KB. *)
let test_extract_addr_size (_ : test_ctxt) : unit =
  ...
  let computation =
    ...
    Exe_info.extract obj proj >>= fun _ ->
    KB.return obj

  in
  let result = KB.run Data.cls computation KB.empty in

  (* The extractor should stash the address size in the KB. *)
...
let suite = [
  "Test Exe_info.extract: stashes address size" >:: test_extract_addr_size;
]
