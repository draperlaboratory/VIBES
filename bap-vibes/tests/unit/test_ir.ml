...
let ( := ) lhs opcode args = simple_op' opcode lhs args
...
let ( vir1,
  ...
  let operands =
    List.map
      ~f:(fun o -> (op_var_exn o).id)
      ( blk1.ins.lhs @ op1.operands @ op1.lhs @ op2.operands @ op2.lhs
      @ blk1.outs.operands )
  in
  let operations =
    List.map ~f:(fun o -> o.id) [ blk1.ins; op1; op2; blk1.outs ]
  in
  let vir1 = { congruent = []; blks = [ blk1 ] } in
  ( vir1,
    definer_map,
    user_map,
    op_opcodes,
...
let test_definer1 _ =
  assert_equal
    ~cmp:(Var.Map.equal equal_op_var)
    definer_map1 (Ir.definer_map vir1)

let test_user1 _ =
  assert_equal
    ~cmp:(Var.Map.equal (List.equal equal_op_var))
    user_map1 (Ir.users_map vir1)

let test_temps1 _ =
  assert_equal ~cmp:Var.Set.equal (Var.Set.of_list temps1) (Ir.all_temps vir1)

let test_operands1 _ =
  assert_equal ~cmp:Var.Set.equal
    (Var.Set.of_list operands1)
    (Ir.all_operands vir1)

let test_op_opcodes1 _ =
  assert_equal
    ~cmp:(Tid.Map.equal (List.equal Ir.equal_opcode))
    op_opcodes1
    (Ir.operation_opcodes vir1)
...
let test_dummy_reg_alloc _ =
  ...
  let all_operations =
    ir_alloc.blks
    |> List.concat_map ~f:(fun b -> (b.ins :: b.outs :: b.data) @ b.ctrl)
  in
  let all_operands =
    all_operations |> List.concat_map ~f:(fun o -> o.lhs @ o.operands)
  in
  let all_op_vars = List.map ~f:Ir.op_var_exn all_operands in
  let all_regs =
    List.map all_op_vars ~f:(fun o -> o.pre_assign) |> List.filter_opt
  in
