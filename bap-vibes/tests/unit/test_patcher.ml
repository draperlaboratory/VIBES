...
let dummy_patch n m : Patcher.patch =
  {
    assembly = [ Printf.sprintf ".rept %Ld" n; "nop"; ".endr" ];
    orig_loc = 0L;
    orig_size = m;
  }
...
let test_patch_placer_exact_fit _ =
  ...
  let patch_sites = [] in
  let placed_patch =
    Patcher.place_patches lang [ patch ] patch_sites |> List.hd_exn
  in
...
let test_patch_placer_no_fit _ =
  ...
  | [ orig_jmp; placed_patch ] ->
      ...
      assert_equal ~printer:Int64.to_string 4L
        (Option.value_exn placed_patch.jmp);
...
let test_patch (_ : test_ctxt) : unit =
  ...
  let computation =
    ...
    >>= fun _ ->
    (* Now run the patcher. *)
    Patcher.patch obj ~patcher >>= fun _ -> KB.return obj
  in
  ...
  H.assert_property
    ...
    ~p_res:H.print_opt ~p_expected:H.print_opt Data.Patched_exe.tmp_filepath
    expected result
...
let test_patch_with_no_original_exe (_ : test_ctxt) : unit =
  ...
  let computation =
    ...
    H.obj () >>= fun obj ->
    Patcher.patch obj >>= fun _ -> KB.return obj
  in
  let result = KB.run Data.cls computation KB.empty in
  ...
  H.assert_error ~printer:H.print_opt Data.Patched_exe.tmp_filepath expected
    result
