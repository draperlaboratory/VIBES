...
let dummy_proj ?(name = "main") filename : (Project.t * string, Error.t) result
    =
  let empty_proj = empty_proj filename in
  let dummy_main = Sub.create ~name () in
  ...
  Result.map empty_proj ~f:(fun p ->
      (Project.with_program p dummy_prog, filename))
...
let extract_property (property : ('k, 'a) KB.slot)
    (result : (('k, 's) KB.cls KB.value * KB.state, KB.conflict) result) : 'a =
  match result with
  | Ok (value, _) -> KB.Value.get property value
  | Error problem ->
      let msg =
        Format.asprintf "@[<v 4>expected a value, but got an error:@,@[%a@]@]"
...
(* After a [kb_run] computation, assert that a property of the returned [obj]
   ...
   value). *)
let assert_property ~cmp ?p_res ?p_expected property expected result : unit =
  let actual = extract_property property result in
  let msg =
    match (p_res, p_expected) with
    | Some p, Some p' ->
        Format.asprintf "@[<v 4>%s:@,@[%s@]@]@.@[<v 4>%s:@,@[%s@]@]" "expected"
          (p' expected) "but got" (p actual)
    | _ -> Format.sprintf "Property did not have the expected value"
  in
      ...
      let msg =
        ...
        | Some p ->
            Format.asprintf "@[<v 4>%s:@,@[%s@]@]@.@[<v 4>%s:@,@[%s@]@]"
              ...
              "but got value" (p actual)
        | None ->
            Format.asprintf "@[<v 4>Got a value, but expected error:@,@[%a@]@]"
...
let print_prog_opt opt =
  match opt with Some prog -> print_prog prog | None -> "None"

(* Pretty print BIR. *)
let print_bir (bir : Insn.t) = Format.asprintf "%a" Insn.pp_adt bir

(* A verifier function for testing. It always returns unsat. *)
let verify_unsat (_ : Bap_wp.Run_parameters.t) (_ : Bap_wp.Runner.input list) :
...
let verify_sat (_ : Bap_wp.Run_parameters.t) (_ : Bap_wp.Runner.input list) :
    (Verifier.status, Bap_main.error) result =
  Result.return Z3.Solver.SATISFIABLE

(* A verifier function for testing. It always returns unknown. *)
let verify_unknown (_ : Bap_wp.Run_parameters.t) (_ : Bap_wp.Runner.input list)
    : (Verifier.status, Bap_main.error) result =
