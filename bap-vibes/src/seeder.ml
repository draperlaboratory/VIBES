...
let extract_patch (p : Data.Patch.t) (s : KB.state) :
  ...
  | Error e ->
      let msg =
        Format.asprintf "(KB.return patch) failed in KB: %a" KB.Conflict.pp e
      in
  ...
  | Ok (value, _) -> (
      ...
      | Some patch_name -> (
          let minizinc_solutions =
            KB.Value.get Data.Patch.minizinc_solutions value
          in
...
let get_patch_by_name (seed : t option) (name : string) : patch option =
  match seed with
  | None -> None
  | Some t -> List.find t.patches ~f:(fun p -> String.equal p.patch_name name)
...
(* Create a patch set. *)
let create_patches ?(seed = None) ~filename ~addr_size (ps : Config.patch list)
  ...
  let create_patch (seed : t option) (p : Config.patch) : Data.Patch.t KB.t =
    ...
    let* lang =
      Utils.get_lang ~filename ~addr_size ~addr:(Config.patch_point p)
    in
    let* tgt =
      Utils.get_target ~filename ~addr_size ~addr:(Config.patch_point p)
    in
    ...
    let* () =
      match get_patch_by_name seed patch_name with
      | None -> KB.return ()
      | Some patch_seed ->
          let* () =
            Data.Patch.union_minizinc_solution obj patch_seed.minizinc_solutions
          in
...
let init_KB ?(seed = None) (config : Config.t) (proj : Bap.Std.Project.t) :
    Data.t KB.t =
  ...
  let* () =
    Data.Solver.set_minizinc_model_filepath obj (Some mzn_model_filepath)
  in
