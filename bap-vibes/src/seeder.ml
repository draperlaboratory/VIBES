...
type patch = {
  ...
  ins_outs_map : Data.ins_outs Bap.Std.Tid.Map.t;
}
...
let extract_patch (p : Data.Patch.t) (s : KB.state)
    ...
    let ins_outs_map = KB.Value.get Data.Patch.ins_outs_map value in
    Some { raw_ir; patch_name; minizinc_solutions; exclude_regs; congruence; ins_outs_map}
  ...
  let create_patch (seed : t option) (p : Config.patch) : Data.Patch.t KB.t =
    ...
    let* () = match get_patch_by_name seed patch_name with
      ...
      | Some patch_seed ->
        ...
        let* () = Data.Patch.set_ins_outs_map obj patch_seed.ins_outs_map in
