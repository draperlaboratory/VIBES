...
type patch = { assembly : string list; orig_loc : int64; orig_size : int64 }
[@@deriving sexp, equal]

(* A [patch_site] is an extra location in the binary where it is
   acceptable to overwrite and place code.  *)
type patch_site = { location : int64; size : int64 } [@@deriving sexp]
...
let binary_of_asm (lang : Theory.language) (assembly : string list) :
  ...
  let args = [ "-o"; with_elf_filename; tgt_flag; asm_filename ] in
  let info_str =
    Format.asprintf "Calling %s %s..." assembler (String.concat ~sep:" " args)
  in
...
let build_patch (l : Theory.language) (patch : placed_patch) :
  ...
  let abs_jmp (abs_addr : int64) : string =
    Printf.sprintf "b (%s + (%Ld))" Constants.patch_start_label abs_addr
  in
  let patch_relative =
    Printf.sprintf ".equiv %s, %Ld\n" Constants.relative_patch_placement
      Int64.(patch.patch_loc - patch.orig_loc)
  in
  let patch_loc =
    Printf.sprintf ".equiv %s, %Ld\n" Constants.patch_location patch.patch_loc
  in
  let patch_start = Printf.sprintf "%s:" Constants.patch_start_label in
  let patch_jmp =
    match patch.jmp with
    | None -> ""
    | Some j -> abs_jmp Int64.(j - patch.patch_loc)
  in
  binary_of_asm l
    ( patch_loc :: patch_relative :: patch_start
...
let patch_file (lang : Theory.language) (original_exe_filename : string)
    (patches : placed_patch list) : string =
  let tmp_patched_exe_filename =
    Stdlib.Filename.temp_file "vibes-assembly" ".patched"
  in
  ...
  Out.with_file tmp_patched_exe_filename ~f:(fun file ->
      ...
      Core_kernel.List.iter patches ~f:(fun patch ->
          Out.seek file patch.patch_loc;
          let patch_binary = build_patch lang patch in
          let patch_binary =
            Result.map_error patch_binary ~f:(Format.asprintf "%a" Kb_error.pp)
          in
...
let naive_find_patch_sites (filename : string) : patch_site list =
  ...
  let command =
    Printf.sprintf
      "objdump %s -dF 2>&1 | grep vibes_dummy | grep -oP \"(?<=File Offset: \
       0x)([0-9a-fA-F]+)\""
      filename
  in
...
let loose_fit_patch (patch : patch) (patch_size : int64) :
...
let external_patch_site (patch : patch) (patch_loc : int64) :
    placed_patch * placed_patch =
  let open Int64 in
  let placed_patch = exact_fit_patch patch in
  let jmp_to_patch =
    { placed_patch with assembly = []; jmp = Some patch_loc }
  in
  let placed_patch =
    {
      placed_patch with
...
let find_site_greedy (patch_sites : patch_site list) (patch_size : int64) :
  ...
  let rec find_site_aux patch_sites =
    ...
    | p :: ps ->
        if p.size >= patch_size then
          let new_patch_site =
            { location = p.location + patch_size; size = p.size - patch_size }
          in
...
*)
let place_patches (lang : Theory.language) (patches : patch list)
  ...
  let process_patch (acc, patch_sites) patch =
    ...
    if patch_size <= patch.orig_size (* Patch fits inplace *) then
      if (* If patch exactly fits *)
         patch_size = patch.orig_size then
        (exact_fit_patch patch :: acc, patch_sites)
      else
        (* Inexact fit. Add jmp, put leftover space in patch_sites *)
        let placed_patch, new_patch_site = loose_fit_patch patch patch_size in
        (placed_patch :: acc, new_patch_site :: patch_sites)
    else
      (* Patch does not fit inplace*)
  ...
  let placed_patches, _ =
    List.fold patches ~init:([], patch_sites) ~f:process_patch
  in
  placed_patches
...
let patch ?(patcher = patch_file) (obj : Data.t) : unit KB.t =
  let open KB.Let in
  ...
  let tmp_patched_exe_filename =
    patcher lang original_exe_filename placed_patches
  in

  (* Stash the filepath in the KB. *)
  let* _ =
    Data.Patched_exe.set_tmp_filepath obj (Some tmp_patched_exe_filename)
  in
  KB.return ()
