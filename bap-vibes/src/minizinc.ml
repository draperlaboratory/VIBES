...
let mznmap_of_yojson _ = [%of_yojson: 'b list]

let mznmap_to_yojson _ = [%to_yojson: 'b list]
...
let mzn_enum_def_of_list (tags : string list) : mzn_enum_def =
  { set = List.map ~f:mzn_enum tags }

let mzn_enum_of_var (v : var) : mzn_enum =
  Var.sexp_of_t v |> Sexp.to_string |> mzn_enum
...
*)
let serialize_mzn_params (tgt : Theory.target) (lang : Theory.language)
    (vir : Ir.t) (prev_sols : sol list) : mzn_params_serial * serialization_info
    =
  ...
  in
  let opcodes =
    Tid.Map.data params.operation_opcodes
    |> List.concat_map ~f:(fun is ->
           List.map is ~f:(fun i ->
               Ir.sexp_of_opcode i |> Ppx_sexp_conv_lib.Sexp.to_string))
    |> String.Set.of_list |> String.Set.to_list
  in
  let blocks =
    Var.Map.data params.temp_block |> Tid.Set.of_list |> Tid.Set.to_list
  in
  let operations = Tid.Map.keys params.operation_opcodes in
  let operands =
    Var.Set.to_list params.operands
    (* Var.Map.keys params.operand_operation *)
  in
  let width t =
    match Var.typ t with
    | Imm n ->
        Float.(of_int n / 32.0 |> round_up |> to_int) (* fishy. Use divmod? *)
      ...
      operand_operation =
        List.map
          ~f:(fun t ->
            ...
            |> Ir.operation_to_string |> mzn_enum)
          operands;
      definer =
        List.map
          ~f:(fun t ->
            Var.Map.find_exn params.definer t |> Ir.op_var_to_string |> mzn_enum)
          temps;
      users =
        List.map
          ~f:(fun t ->
            ...
            | Some operands ->
                {
                  set =
                    List.map
                      ~f:(fun o -> Ir.op_var_to_string o |> mzn_enum)
                      operands;
                })
          temps;
      temp_block =
        List.map temps ~f:(fun t ->
            Var.Map.find_exn params.temp_block t |> Tid.to_string |> mzn_enum);
      copy =
        {
          set =
      ...
      operation_opcodes =
        List.map
          ~f:(fun o ->
            {
              set =
                Tid.Map.find_exn params.operation_opcodes o
                |> List.map ~f:(fun i ->
                       Ir.sexp_of_opcode i |> Ppx_sexp_conv_lib.Sexp.to_string
                       |> mzn_enum);
            })
          operations;
      latency = List.map ~f:(fun _ -> 10) opcodes;
      (* TODO *)
      number_excluded = List.length prev_sols;
      exclude_reg =
        List.map prev_sols ~f:(fun sol ->
            List.map temps ~f:(fun t ->
                Var.Map.find_exn sol.reg t |> mzn_enum_of_var));
    },
    {
      temps;
...
let apply_sol (vir : Ir.t) (sol : sol) : Ir.t =
  ...
  let vir =
    Ir.map_blks vir ~f:(fun b ->
        {
          b with
          data =
            List.filter b.data ~f:(fun o -> Tid.Map.find_exn sol.active o.id)
            |> List.sort ~compare:(fun o1 o2 ->
                   compare_int
...
let delete_empty_blocks vir =
  ...
  let blks =
    List.fold blks ~init:[] ~f:(fun acc b ->
        if List.is_empty b.data && List.is_empty b.ctrl then acc else b :: acc)
  in
  { vir with blks }

let run_minizinc (tgt : Theory.target) (lang : Theory.language)
    ~filepath:(model_filepath : string) (prev_sols : sol list) (vir : Ir.t) :
    (Ir.t * sol) KB.t =
  let params_filepath = Stdlib.Filename.temp_file "vibes-mzn-params" ".json" in
  let solution_filepath = Stdlib.Filename.temp_file "vibes-mzn-sol" ".json" in
  Events.(send @@ Info (sprintf "Paramfile: %s\n" params_filepath));
  Events.(
    send
    @@ Info
         (sprintf "Number of Excluded Solutions: %d\n" (List.length prev_sols)));
  ...
  let minizinc_args =
    [
      ...
      "--soln-sep";
      "\"\"";
      (* Suppress some unwanted annotations *)
      "--search-complete-msg";
      "\"\"";
      model_filepath;
    ]
  in
  Utils.lift_kb_result (Utils.run_process "minizinc" minizinc_args)
  >>= fun () ->
  let sol_serial =
    Yojson.Safe.from_file solution_filepath |> sol_serial_of_yojson
  in
  let sol =
    match sol_serial with
    | Ok sol_serial -> KB.return (deserialize_sol sol_serial name_maps)
    | Error msg -> Kb_error.fail (Kb_error.Minizinc_deserialization msg)
  in
