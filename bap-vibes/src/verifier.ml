...
type verifier = Params.t -> Runner.input list -> (status, Bap_main.error) result
...
let wp_verifier (p : Params.t) inputs = Runner.run p inputs
...
let verify ?(verifier = wp_verifier) ~(orig_prog : Program.t * string)
    ~(patch_prog : Program.t * string) (tgt : Theory.target) ~(func : string)
    (property : Sexp.t) : (next_step, Toplevel_error.t) result =
  ...
  let param =
    { param with postcond = post; ext_solver_path = Some "boolector" }
  in
  ...
  let* status =
    verifier param [ input1; input2 ]
    |> Result.map_error ~f:(fun e -> Toplevel_error.WP_failure e)
  in
  match status with
  | Z3.Solver.UNSATISFIABLE ->
      Events.(send @@ Info "Weakest-precondition analysis returned: correct");
      Events.(send @@ Info "The patched binary is correct");
      Ok Done
  | Z3.Solver.SATISFIABLE ->
      Events.(send @@ Info "Weakest-precondition analysis returned: incorrect");
