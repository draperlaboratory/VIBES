...
let optimized solver = function
  | ir ->
      let+ ir, sol = solver ir in
      (Arm.peephole ir, sol)

(* Converts a list of BIR statements to a list of ARM assembly strings. *)
let create_assembly (solver : Ir.t -> (Ir.t * Minizinc.sol) KB.t) (ir : Ir.t) :
...
let create_vibes_ir (lang : Theory.language) (bir : Insn.t) : Ir.t KB.t =
  ...
  let arm_eff =
    Result.of_option arm_eff ~error:(Kb_error.Missing_semantics err)
  in
  let ir = Result.map ~f:Arm.ir arm_eff |> Result.map ~f:(Arm.preassign lang) in
  match ir with Ok ir -> KB.return ir | Error e -> Kb_error.fail e

(* Compile one patch from BIR to VIBES IR *)
let compile_one_vibes_ir (count : int KB.t) (patch : Data.Patch.t) : int KB.t =
  count >>= fun n ->
  let info_str =
    Format.asprintf "Translating patch %s BIR to VIBES IR..." (string_of_int n)
  in
...
let compile_one_assembly
    (solver :
      ...
      Minizinc.sol list ->
      Ir.t ->
      (Ir.t * Minizinc.sol) KB.t) (count : int KB.t) (patch : Data.Patch.t) :
    int KB.t =
  count >>= fun n ->
  let info_str =
  ...
  create_assembly (solver target lang prev_sols) ir
  >>= fun (assembly, new_sol) ->
  ...
  Events.(send @@ Rule);
  Data.Patch.add_minizinc_solution patch new_sol >>= fun () -> KB.return (n + 1)
...
let compile_ir (obj : Data.t) : unit KB.t =
  ...
  Data.Patch_set.fold patches ~init:(KB.return 1) ~f:compile_one_vibes_ir
  >>= fun _ ->
  Events.(send @@ Info "Done.");
  KB.return ()

(* Converts the patch (as IR) to assembly instructions. *)
let compile_assembly ?(solver = Minizinc.run_minizinc) (obj : Data.t) :
    unit KB.t =
  ...
  Data.Patch_set.fold patches ~init:(KB.return 1)
    ~f:(compile_one_assembly (solver ~filepath:mzn_model))
  >>= fun _ ->
  Events.(send @@ Info "Done.");
  KB.return ()
