...
module CoreParser (Core : Theory.Core) = struct
  ...
  let parse_var_decls (nm : string) (word_t : vsort) (ds : Sexp.t) :
    ...
    let default_vars : (string * vvar) list = [] in
    ...
    let var_assocs : (string * vvar) list =
      default_vars @ List.map ~f:(fun s -> (s, Var.define word_t s)) var_strings
    in
    match M.of_alist var_assocs with
    | `Duplicate_key k ->
        Kb_error.fail
          (named_err nm
             ( "has duplicated or reserved variable \"" ^ k
             ^ "\" in declaration list" ))
    | `Ok m -> KB.return m
  ...
  let parse_int (st : parse_state) (s : string) : int KB.t =
    ...
    with _ -> (
      try
        ...
        KB.return (Scanf.sscanf s "%d" (fun x -> x))
      with _ ->
        (* neither worked *)
        Kb_error.fail
          (named_err st.nm
  ...
  let rec parse_pure (st : parse_state) (p : Sexp.t) : vpure KB.t =
    ...
    | _ ->
        Kb_error.fail
          (named_err st.nm ("contains invalid value " ^ Sexp.to_string p))

  let parse_stmt (st : parse_state) (sexps : Sexp.t) : data eff KB.t =
    match sexps with
    | Sexp.Atom s ->
        Kb_error.fail
          (named_err st.nm ("uses unimplemented atomic statement " ^ s))
    ...
    | Sexp.List (Sexp.List _ :: _) ->
        Kb_error.fail
          (named_err st.nm "has an invalid statement (list at head)")
  ...
  let parse_bir (nm : string) (bits : int) (sexps : Sexp.t list) : insn KB.t =
    ...
    | decls :: sexps ->
        ...
        let* (data : data eff list) =
          KB.all (List.map ~f:(parse_stmt st) stmt_sexps)
        in
        let data_blk =
...
let ingest (obj : Data.t) : unit KB.t =
  ...
  Data.Patch_set.fold patches ~init:(KB.return 1) ~f:(ingest_one addr_size)
  >>= fun _ ->
