...
let string_domain : String.t option KB.Domain.t =
  KB.Domain.optional ~equal:String.( = ) "string-domain"

(* Optional int domain *)
let int_domain : int option KB.Domain.t =
  KB.Domain.optional ~equal:Int.( = ) "int-domain"

(* Optional bitvector domain *)
let bitvec_domain : Bitvec.t option KB.Domain.t =
  KB.Domain.optional ~equal:Bitvec.equal "bitvec-domain"
...
let sexp_domain : Sexp.t option KB.Domain.t =
  KB.Domain.optional ~equal:Sexp.equal "sexp-domain"

(* Optional s-expression list domain (e.g., for patch code) *)
let sexp_list_domain : Sexp.t list option KB.Domain.t =
  KB.Domain.optional
...
let assembly_domain : string list option KB.Domain.t =
  KB.Domain.optional
...
let ir_domain : Ir.t option KB.Domain.t =
  KB.Domain.optional ~equal:Ir.equal "ir-domain"
...
module Patch = struct
  ...
  let patch : (patch_cls, unit) KB.cls = KB.Class.declare ~package "patch" ()
  ...
  let get_patch_name (obj : t) : string option KB.t = KB.collect patch_name obj
  ...
  let get_patch_size (obj : t) : int option KB.t = KB.collect patch_size obj
  ...
  let set_bir (obj : t) (data : Insn.t) : unit KB.t = KB.provide bir obj data

  let get_bir (obj : t) : Insn.t KB.t = KB.collect bir obj

  let set_lang (obj : t) (data : Theory.language) : unit KB.t =
    KB.provide lang obj data

  let get_lang (obj : t) : Theory.language KB.t = KB.collect lang obj

  let set_target (obj : t) (data : Theory.target) : unit KB.t =
    KB.provide target obj data

  let get_target (obj : t) : Theory.target KB.t = KB.collect target obj

  let set_raw_ir (obj : t) (data : Ir.t option) : unit KB.t =
    KB.provide raw_ir obj data

  let get_raw_ir (obj : t) : Ir.t option KB.t = KB.collect raw_ir obj
  ...
  let get_assembly (obj : t) : string list option KB.t = KB.collect assembly obj
...
module Original_exe = struct
  let filepath : (cls, string option) KB.slot =
    KB.Class.property ~package cls "original-exe-filepath" string_domain

  let addr_size : (cls, int option) KB.slot =
    KB.Class.property ~package cls "original-exe-address-size" int_domain

  let set_filepath (obj : t) (data : string option) : unit KB.t =
    KB.provide filepath obj data

  let get_filepath (obj : t) : string option KB.t = KB.collect filepath obj
  ...
  let get_addr_size (obj : t) : int option KB.t = KB.collect addr_size obj
...
module Patched_exe = struct
  ...
  let filepath : (cls, string option) KB.slot =
    KB.Class.property ~package cls "patched-exe-filepath" string_domain

  let tmp_filepath : (cls, string option) KB.slot =
    KB.Class.property ~package cls "tmp-patched-exe-filepath" string_domain

  let set_patches (obj : t) (data : Patch_set.t) : unit KB.t =
    KB.provide patches obj data

  let get_patches (obj : t) : Patch_set.t KB.t = KB.collect patches obj
...
module Solver = struct
  ...
  let set_minizinc_model_filepath (obj : t) (data : string option) : unit KB.t =
...
module Verifier = struct
  ...
  let get_property (obj : t) : Sexp.t option KB.t = KB.collect property obj
  ...
  let get_func (obj : t) : string option KB.t = KB.collect func obj
