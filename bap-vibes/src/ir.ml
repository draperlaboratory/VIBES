...
module Opcode = struct
  ...
  let create ?arch s = Knowledge.Name.create ?package:arch s
...
type op_var = { id : Var.t; temps : Var.t list; pre_assign : var option }
      ...
      let fresh_v =
        {
          v with
          id =
            Var.create ~is_virtual:true ~fresh:true (Var.name v.id)
              (Var.typ v.id);
        }
      in
      Var fresh_v
...
let op_var_exn (x : operand) : op_var =
  match x with Var o -> o | _ -> failwith "Expected op_var"
...
let mk_empty_operation () =
  let tid = Tid.create () in
  { id = tid; lhs = []; opcodes = []; optional = false; operands = [] }
...
let union t1 t2 =
  ...
  {
    blks = List.dedup_and_sort ~compare:compare_blk (t1.blks @ t2.blks);
    congruent =
      List.dedup_and_sort ~compare:comp_pair (t1.congruent @ t2.congruent);
  }

let add blk t = { t with blks = blk :: t.blks }
...
module Blk = struct
  ...
  let all_rhs_operands (blk : blk) : operand list =
    let operation_operands op_list =
      List.concat_map op_list ~f:(fun operation -> operation.operands)
    in
    blk.ins.operands @ blk.outs.operands
    @ operation_operands blk.data
    @ operation_operands blk.ctrl

  let all_lhs_operands (blk : blk) : operand list =
    let operation_operands op_list =
      List.concat_map op_list ~f:(fun operation -> operation.lhs)
    in
    blk.ins.lhs @ blk.outs.lhs
    @ operation_operands blk.data
    @ operation_operands blk.ctrl

  let all_temps (blk : blk) : Var.Set.t =
    List.concat_map (all_operands blk) ~f:(fun op ->
        match op with
  ...
  let operation_opcode (blk : blk) : opcode list Tid.Map.t =
    List.fold ~init:Tid.Map.empty
      ~f:(fun acc o -> Tid.Map.add_exn acc ~key:o.id ~data:o.opcodes)
      (all_operations blk)

  let operand_operation (blk : blk) : operation Var.Map.t =
    let alist =
      List.concat_map (all_operations blk) ~f:(fun operation ->
          let rhs_operands =
            List.map (var_operands operation.operands) ~f:(fun op ->
                (op.id, operation))
          in
          let lhs_operands =
            List.map (var_operands operation.lhs) ~f:(fun op ->
                (op.id, operation))
          in
...
let var_map_union_exn m1 m2 =
  Var.Map.merge m1 m2 ~f:(fun ~key:_ ab ->
      match ab with
      | `Left a -> Some a
      | `Right b -> Some b
...
let tid_map_union_exn m1 m2 =
  Tid.Map.merge m1 m2 ~f:(fun ~key:_ ab ->
      match ab with
      | `Left a -> Some a
      | `Right b -> Some b
      | `Both (_, _) -> failwith "Map has both keys")
...
let map_op_vars ~f (vir : t) : t =
  let f2 o = match o with Var o -> Var (f o) | o -> o in
  let apply_to_op (o : operation) : operation =
    { o with lhs = List.map ~f:f2 o.lhs; operands = List.map ~f:f2 o.operands }
  in
  {
    blks =
      List.map vir.blks ~f:(fun b ->
          {
            b with
    ...
    congruent = List.map ~f:(Tuple2.map ~f) vir.congruent;
  }

let all_temps (sub : t) : Var.Set.t =
  List.fold sub.blks ~init:Var.Set.empty ~f:(fun acc blk ->
...
let temp_blk (sub : t) : Tid.t Var.Map.t =
  List.concat_map sub.blks ~f:(fun blk ->
      List.map (Blk.all_temps blk |> Var.Set.to_list) ~f:(fun t -> (t, blk.id)))
  |> Var.Map.of_alist_exn

let operation_opcodes (sub : t) : opcode list Tid.Map.t =
  List.fold sub.blks ~init:Tid.Map.empty ~f:(fun acc blk ->
...
let pretty_operand o =
  match o with
  | Var o ->
      sprintf "%s : %s < %s" (Var.to_string o.id)
        (List.map ~f:Var.to_string o.temps |> String.concat ~sep:"::")
        (Option.map ~f:Var.to_string o.pre_assign |> Option.value ~default:"N/A")
...
let pretty_operation o =
  sprintf "\t\t[%s]  <- %s [%s]"
    ...
    (String.concat (List.map o.opcodes ~f:Opcode.name))
    (pretty_operand_list o.operands)

let pretty_blk b =
  sprintf "blk : %s \n\tins : %s \n\touts: %s\n\tdata: \n%s\n\tctrl: \n%s"
    (Tid.to_string b.id) (pretty_operation b.ins) (pretty_operation b.outs)
    (List.fold b.data ~init:"" ~f:(fun acc o -> acc ^ pretty_operation o ^ "\n"))
    (List.fold b.ctrl ~init:"" ~f:(fun acc o -> acc ^ pretty_operation o ^ "\n"))
