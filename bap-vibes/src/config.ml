...
type t = {
  exe : string;
  (* The filename (path) of the executable to patch. *)
  patches : patch list;
  ...
  (* Optional number of CEGIS iterations to allow *)
...
let patch_to_string (p : patch) : string =
  let code =
    String.concat ~sep:"\n" (List.map p.patch_code ~f:Sexp.to_string)
  in
  String.concat ~sep:"\n"
    [
