...
let halt_if_too_many (count : int) (max_tries : int option) :
  ...
  | None -> Ok ()
  | Some 0 -> Ok ()
  | Some n -> if count > n then Error (Toplevel_error.Max_tries n) else Ok ()
...
let create_patched_exe ~(seed : Seeder.t) (config : Config.t) (proj : project) :
    Data.t KB.t =
...
let get_tmp_patched_exe_filepath (value : Data.computed) :
  ...
  | None ->
      let msg = "KB is missing a filepath for the temporary patched exe" in
...
let finalize_patched_exe (value : Data.computed) :
  ...
  | Some orig_path, Some tmp_path ->
      ...
      Events.(
        send @@ Info (Printf.sprintf "Patched exe: %s\n " patched_exe_filepath));
      Ok patched_exe_filepath
  | None, _ ->
      let msg = "KB is missing a filepath for the original exe" in
      Error (Toplevel_error.No_value_in_KB msg)
  | _, None ->
      let msg = "When finalizing patched exe, no tmp filepath found in KB" in
      Error (Toplevel_error.No_value_in_KB msg)
...
let rec cegis ?(count = 1) ?(max_tries = None) ~(seed : Seeder.t)
  ...
  let verif_res =
    Verifier.verify target ~func property
  ...
  | Ok Verifier.Again ->
      ...
      cegis config orig_proj orig_prog state ~count:new_count ~max_tries
        ~seed:new_seed
  | Error e -> Error e
