...
(** This is copied verbatim from the BAP flatten plugin.
...
let flatten_exp (exp : exp) (blk : blk term) (before : tid) : (exp * blk term) KB.t =
  let is_virtual = true in
  let fresh = true in
  let rec aux (exp : exp) (blk : blk term) : (exp * blk term) KB.t = 
    ...
    | Bil.Load (x, y, endian, s) ->
      ...
      let def = Def.create var e ~tid in
      Bil.Var var,
