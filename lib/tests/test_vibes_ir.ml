...
let vir1, definer_map1, user_map1, op_insns1,temps1, operands1, operations1, oprnd_temps1 =
  ...
  let definer_map = Var.Map.of_alist_exn [  (t1, op_var_exn (List.hd_exn  blk1.ins.lhs)) ; (t2, op_var_exn (List.hd_exn op1.lhs)); (t3, op_var_exn (List.hd_exn op2.lhs)) ] in
  let user_map = Var.Map.of_alist_exn [  (t1, List.map ~f:op_var_exn op1.operands)  ; (t2, List.map ~f:op_var_exn op2.operands); (t3, List.map ~f:op_var_exn  blk1.outs.operands) ] in
  let op_insns = Tid.Map.of_alist_exn [  (op1.id, [`MOVi]  ) ; (op2.id, [`MOVi] ) ; ( blk1.ins.id, []) ; (blk1.outs.id, []) ] in 
  let oprnd_temps = [ t1 ; t1; t2; t2 ; t3; t3 ] in
  let operands = List.map ~f:(fun o -> (op_var_exn o).id ) ( blk1.ins.lhs @  op1.operands  @ op1.lhs @ op2.operands @ op2.lhs @ blk1.outs.operands) in
