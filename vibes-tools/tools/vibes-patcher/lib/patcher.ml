...
module Err = struct
  ...
  let _fail s = failwith s
end

module Events = struct
  type t = Info of string | Header of string
  let send = function
   | Info s -> print_endline s
   | Header s -> print_endline s
end
...
let check_for_literal_pool (l : Theory.language) (assembly : string list)
  ...
  let* _ = Result.map_error ~f:(fun s -> Option.value_exn (Utils.Types.printer s)) @@
           Utils.Proc.run assembler args in
...
let binary_of_elf (with_elf_filename : string) : (string, Err.t) Result.t =
  let (let*) x f = Result.bind x ~f in
  (* strip elf data *)
  ...
  in
  let* _ = Result.map_error ~f:(fun s -> Option.value_exn (Utils.Types.printer s)) @@
           Utils.Proc.run objcopy objcopy_args in
        ...
        Core_kernel.List.iter patches
          ~f:(fun patch ->
              ...
              let patch_binary =
                ...
                Result.map_error ~f:(Format.asprintf "%a" Err.pp) |>
...
module Config = struct
  ...
  type t = {
    filename : string;
    patches : patch0 list;
    patch_sites : patch_site list;
    target : Theory.Target.t [@of_yojson fun s -> Ok (Theory.Target.read (Yojson.Safe.Util.to_string s))];
    lang : Theory.Language.t [@of_yojson fun s -> Ok (Theory.Language.read (Yojson.Safe.Util.to_string s))];
  } [@@deriving of_yojson]
end
...
let _reify_patch
    ...
    ~exe_unit:(exe_unit)
    ({assembly; orig_addr; orig_size} : Config.patch0) : patch KB.t =
  let open KB.Let in
  let* addr = Theory.Label.for_addr Bitvec.((int64 orig_addr) mod m64) in
  ...
  let patch_region = compute_region ~loc:orig_addr spec in
  let* {region_addr; region_offset} = match patch_region with
  | Error s -> Err.fail (Core_kernel.Error.to_string_hum s)
  | Ok c -> KB.return c
  in
  ...
  let patch_file_offset = Int64.(orig_addr - region_addr + region_offset) in
  KB.return { assembly;
              orig_loc = patch_file_offset;
              orig_size}
*)
let convert_addr ({assembly; orig_addr; orig_size} : Config.patch0) : patch = 
  { assembly;
    orig_loc = orig_addr;
    orig_size}
...
let patch
    ?compute_region:(_compute_region=ogre_compute_region)
  ...
  (* TODO: fix or replace
  let open KB.Let in
  let* original_exe_unit = Theory.Unit.for_file config.filename in
  let reify_patch = reify_patch
    ~compute_region:compute_region ~exe_unit:original_exe_unit in
  let* patch_list = KB.List.map ~f:reify_patch config.patches in *)
  let patch_list = List.map ~f:convert_addr config.patches in
  let naive_patch_sites = naive_find_patch_sites config.filename in
  let patch_sites = naive_patch_sites @ config.patch_sites in
  ...
  let placed_patches = place_patches config.target config.lang patch_list patch_sites in
  ...
  let tmp_patched_exe_filename = patcher config.lang ~filename:config.filename placed_patches in

  Events.(send @@ Info (Format.asprintf "patched filename: %s" tmp_patched_exe_filename));
  ()
