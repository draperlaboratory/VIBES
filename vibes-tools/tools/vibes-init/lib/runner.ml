...
let collect_ogre (binary : string) : (string, KB.conflict) result =
  Log.send "Generating OGRE file %s for binary %s" Types.default_ogre binary;
  let* image = Vibes_patch.Loader.image binary in
  let data = Format.asprintf "%a" Ogre.Doc.pp @@ Image.spec image in
  let* () = Files.write_or_error data Types.default_ogre in
  Ok Types.default_ogre
...
let run
  ...
  Log.send "Vibes_init.Runner.run '%s' '%s' '%s' '%s' '%s' '%s' '%s'"
    ...
    (Option.value ogre_filepath ~default:Types.default_ogre);
  ...
  let t =
    Types.create target language patch_names
      ...
      ~spaces:Types.default_patch_spaces in
