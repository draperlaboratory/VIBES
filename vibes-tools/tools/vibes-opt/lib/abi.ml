...
let collect_argument_tids
    (sub : sub term)
    ~(target : T.target)
    ~(func_info : Function_info.t) : Tid.Set.t KB.t =
  ...
  *)
  ...
  let create_activation_record
    ...
    let push v =
      ...
      let+ tid = T.Label.fresh in
      Def.create ~tid mem @@
      Store (Var mem, addr, Var reg, endian, `r32) in
    ...
    match Bir_helpers.exit_blks sub with
    ...
    | Ok exits ->
      ...
      let+ sub = Term.KB.map blk_t sub ~f:(fun blk ->
          ...
          if Tid.(tid = entry_tid) then
            ...
            let+ tid = T.Label.fresh in
            let adj =
              Def.create ~tid sp @@ BinOp (MINUS, Var sp, Int space) in
            ...
            let+ tid = T.Label.fresh in
            let adj =
              Def.create ~tid sp @@ BinOp (PLUS, Var sp, Int space) in
  ...
  let spill_hvars_and_adjust_stack
    ...
    then !!{sub; hvars; spilled = String.Set.empty}
      ...
      let live_after_call v = List.exists calls ~f:(fun blk ->
